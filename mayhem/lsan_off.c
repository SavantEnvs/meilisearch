/* mayhem/lsan_off.c — disable LeakSanitizer at BUILD time for the fuzz target.
 *
 ...
 * the bug class this fleet fuzzes for, and LSan's at-exit scan is both noisy on Rust
 * arena / `Box::leak` patterns and conflicts with Mayhem's coverage tracer.
 *
 ...
 *
 * mayhem/build.sh compiles this TU and prepends the object to every rustc link via a
 * generated -Clinker wrapper (rustc accepts only ONE -Clinker), so it lands in the
 * fuzz binary. ASan and UBSan stay on and halting.
 */
